...
async function migrateJLuis() {
  const iesId = 'ies_rey_fernando';
  const jluisId = 'd8ArhHgHLGWKGmYCgRgp7Titexv2';
  ...
  const subjectsMapping = {
    'Desarrollo de aplicaciones web en entorno cliente': { sigla: 'DWEC', id: 'iAwH7ltSr9UDWbSZkJui' }
  };

  console.log('--- CLEANING PREVIOUS DATA FOR JOSÉ LUIS ---');
  const impSnap = await db.collection('ies_imparticiones')
    .where('usuarioId', '==', jluisId)
  ...
  const segXml = fs.readFileSync('legacy/programaciones/dat/seguimiento-JLuis.xml', 'utf-8');
  ...
  for (const dept of depts) {
    ...
    for (const curso of cursos) {
      ...
      for (const asig of asigs) {
        if (asig.grupo && (asig.grupo.profe === 'JLuis' || asig.grupo.nif === 'JLuis')) {
          ...
          const impId = `${cursoAcademicoLabel.replace('-', '')}_${groupInfo.nombre}_${subInfo.sigla}_JL`.replace(/\s+/g, '');
          
          await db.collection('ies_imparticiones').doc(impId).set({
            iesId,
            usuarioId: jluisId,
            profesorNombre: 'José Luis Gallego García',
            cursoAcademicoId,
            cursoAcademicoLabel,
...
migrateJLuis().then(() => {
  console.log('--- JOSÉ LUIS MIGRATION COMPLETED SUCCESSFULLY ---');
  process.exit(0);
}).catch(err => {
