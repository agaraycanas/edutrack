...
const UID = 'ZVmODO1waPSlSgFCJ6LlBnshR002'; // María José Madrid
...
const asigMap = {
  "Entornos de desarrollo": {
    "m1d": { id: "PY8KPS52CIh7kgXw4RLX", sigla: "ED", studyId: studiesMap.DAM },
    "w1": { id: "DvulyDA1aBmRYRYALJye", sigla: "ED", studyId: studiesMap.DAW }
  },
  "Lenguajes de marcas": {
    "m1d": { id: "NIU8w80WO5TR4re5iOW7", sigla: "LMSGI", studyId: studiesMap.DAM },
    "w1": { id: "5fCajSLBDiNTjceHLrpB", sigla: "LM", studyId: studiesMap.DAW }
  }
};
...
async function migrateMJose() {
  console.log("--- Iniciando migración de María José Madrid ---");

  const progXml = fs.readFileSync('legacy/programaciones/dat/programaciones.xml', 'utf-8');
  
  const assignments = [
    { asig: "Entornos de desarrollo", grupo: "m1d", targetGroup: "DAM1D", schedule: { Mon: 0, Tue: 1, Wed: 1, Thu: 0, Fri: 0 } },
    { asig: "Lenguajes de marcas", grupo: "m1d", targetGroup: "DAM1D", schedule: { Mon: 1, Tue: 0, Wed: 0, Thu: 0, Fri: 2 } },
    { asig: "Entornos de desarrollo", grupo: "w1", targetGroup: "DAW1", schedule: { Mon: 0, Tue: 1, Wed: 1, Thu: 0, Fri: 0 } },
    { asig: "Lenguajes de marcas", grupo: "w1", targetGroup: "DAW1", schedule: { Mon: 2, Tue: 0, Wed: 0, Thu: 0, Fri: 1 } }
  ];
  ...
  for (const ass of assignments) {
    ...
    const impRef = await db.collection('ies_imparticiones').add({
      ...
      usuarioId: UID,
      profesorNombre: 'María José Madrid',
      departamento: DEPT,
    ...
    const segXml = fs.readFileSync('legacy/programaciones/dat/seguimiento-MJose.xml', 'utf-8');
  ...
  console.log("\n--- Migración de María José Madrid completada ---");
  process.exit(0);
}

migrateMJose().catch(console.error);
